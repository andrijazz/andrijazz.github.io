...
function IconGitHub() {
  return (
    <svg aria-hidden width={14} height={14} viewBox="0 0 24 24" className="social-icon">
      <path
        fill="currentColor"
...
export function SocialLinks({ config }: { config: SiteConfig }) {
  ...
  if (social.github)
    items.push({ href: social.github, label: "GitHub", icon: <IconGitHub /> });
  if (social.twitter)
    items.push({ href: social.twitter, label: "Twitter", icon: <IconX /> });
  if (social.linkedin)
    items.push({ href: social.linkedin, label: "LinkedIn", icon: <IconLinkedIn /> });
  if (social.scholar)
    items.push({ href: social.scholar, label: "Scholar", icon: <IconScholar /> });

  if (items.length === 0) return null;

  return (
    <div className="social-block social-footer">
      <span className="social-label">Find me on:</span>{" "}
      {items.map((item, i) => (
        <span key={item.href}>
          {i > 0 ? " " : null}
          <a href={item.href} className="social-link" rel="me noreferrer noopener" target="_blank">
            {item.icon}
